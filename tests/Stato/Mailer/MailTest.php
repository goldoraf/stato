...
class Stato_Mailer_MailTest extends Stato_TestCase
{
...
EOT;
    ...
    public function testMessageWithoutAContentShouldThrow()
    {
        $mail = new Stato_Mailer_Mail($this->date);
        $mail->addTo('john.doe@fake.net', 'John Doe');
        $this->setExpectedException('Stato_Mailer_Exception', 'No body specified');
        $mail->getContent();
    }
...
EOT;
    ...
    {
        $mail = new Stato_Mailer_Mail($this->date);
        $this->setExpectedException('Stato_Mailer_Exception', 'To: recipient is not specified');
        $mail->getTo();
    }
...
EOT;
        ...
        $mail->addTo('john.doe@fake.net', 'John Doe');
        $mail->setText('test');
        $this->assertEquals($message, $mail->send(new Stato_Mailer_Transport_Dummy()));
    }
}
