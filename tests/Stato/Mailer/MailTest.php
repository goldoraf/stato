...
EOT;
    ...
    {
        $mail = new Mail($this->date);
        $this->setExpectedException('Stato\Mailer\Exception', 'To: recipient is not specified');
        $mail->getTo();
    }
...
EOT;
        ...
        $mail->addTo('john.doe@fake.net', 'John Doe');
        $mail->setText('test');
        $mail->addAttachment(file_get_contents(__DIR__ . '/files/image.png'), 'hello.png', 'image/png');
        $mail->setBoundary('c67476988f320ca04d61815bcfd14360');
        $this->assertEquals($message, (string) $mail);
    }
...
--c67476988f320ca04d61815bcfd14361--
EOT;
        $mp = new Mime\Multipart();
        $mp->setBoundary('c67476988f320ca04d61815bcfd14360');
        $mp->addPart(new Mime\Part('test'));
        $mp->addPart(new Mime\Part('<b>test</b>', 'text/html'));
        $mail = new Mail($this->date);
        $mail->addTo('john.doe@fake.net', 'John Doe');
        $mail->setContent($mp);
        $mail->addAttachment(file_get_contents(__DIR__ . '/files/image.png'), 'hello.png', 'image/png');
...
EOT;
        ...
        $mail->addTo('john.doe@fake.net', 'John Doe');
        $mail->setText('test');
        $mail->addEmbeddedImage(file_get_contents(__DIR__ . '/files/image.png'), 'hello', 'hello.png', 'image/png');
        $mail->setBoundary('c67476988f320ca04d61815bcfd14360');
        $this->assertEquals($message, (string) $mail);
    }
