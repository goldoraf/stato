...
class OptionParser
{
    ...
    public function addOption($short, $long, $type = Option::BOOLEAN, $dest = null, $help = null, $metavar = null)
    {
        $option = new Option($short, $long, $type, $dest, $help, $metavar);
                ...
                if ($opt{1} == '-') {
                    ...
                    if (strlen($opt) > 2) {
                        if ($potentialValue !== null)
                            throw new Exception("multiple options $opt can't be foolowed by an argument");
    ...
    private function addOptionObject(Option $option)
    {
        $this->options[] = $option;
    ...
    private function parseLongOption($opt, $potentialValue = null)
    {
        if (strpos($opt, '=') !== false) {
            if ($potentialValue !== null)
                throw new Exception("option $opt has been provided with 2 arguments");
    ...
    private function searchOption($opt, $type = self::SHORT)
    {
        ...
        if ($k === false) throw new Exception("unrecognized option $opt");
        return $this->options[$k];
    }
    
    private function processOptionValue($opt, Option $option, $potentialValue = null)
    {
