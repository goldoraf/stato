...
class Stato_Mailer_Mime_Exception extends Exception {}

/**
 * Tooling class for MultiPart Stato_Mailer_Mime_Mime Messages
 *
 * @package    Stato
 * @subpackage Mailer
 */
class Stato_Mailer_Mime_Mime
{
     ...
     * @param int $LineLength; defaults to Stato_Mailer_Mime_Mime::LINE_LENGTH (72)
     * @param string $eol EOL string; defaults to Stato_Mailer_Mime_Mime::EOL (\n)
     * @return string
     */
    public static function encode($content, $encoding, $lineLength = self::LINE_LENGTH, $eol = self::EOL)
    {
        if (!self::isEncodingSupported($encoding))
            throw new Stato_Mailer_Mime_Exception("Not supported encoding: $encoding");
     ...
     * 
     * If the given encoding stream filter is not available, an Stato_Mailer_Mime_Exception
     * will be thrown.
     *
     * @param string $stream
     * @param string $encoding
