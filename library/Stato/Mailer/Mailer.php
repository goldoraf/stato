 ...
 * <code>
 * Stato_Mailer_Mailer::setTemplateRoot('/path/to/msg/templates');
 * 
 * class UserMailer extends Stato_Mailer_Mailer
 * {
 ...
 * <code>
 * $transport = new Stato_Mailer_Transport_Smtp();
 * Stato_Mailer_Mailer::setDefaultTransport($transport);
 * </code>
 *
 * @package Stato
 * @subpackage Stato_Mailer_Mailer
 */
class Stato_Mailer_Mailer
{
    ...
    public static function setDefaultTransport(Stato_Mailer_Transport_ITransport $transport)
    {
    ...
    public function __call($methodName, $args)
    {
        ...
        throw new Stato_Mailer_Exception(get_class($this)."::$methodName() method does not exist");
    }
    ...
    public function prepare($methodName, $args)
    {
        ...
        if (!($mail instanceof Stato_Mailer_Mail)) $mail = $this->create($methodName, $args);
        return $mail;
    }
    ...
    {
        $mail = new Stato_Mailer_Mail($this->date);
    ...
    protected function render($templateName, $locals = array())
    {
        ...
        if (!file_exists($templatePath) || !is_readable($templatePath))
            throw new Stato_Mailer_Exception("Missing template $templatePath");
    ...
    protected function getTemplatePath($templateName)
    {
        ...
        if (!isset(self::$templateRoot))
            throw new Stato_Mailer_Exception('Template root not set');
    ...
    {
        return (isset(self::$transport)) ? self::$transport : new Stato_Mailer_Transport_Sendmail();
    }
