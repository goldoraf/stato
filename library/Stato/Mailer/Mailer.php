 ...
 * <code>
 * Mailer::setTemplateRoot('/path/to/msg/templates');
 * 
...
class Mailer
{
    ...
    public static function setDefaultTransport(Transport\ITransport $transport)
    {
    ...
    public static function __callStatic($methodName, $args)
    {
        ...
        if (preg_match('/^send([a-zA-Z0-9_]*)$/', $methodName, $m))
            return $mailer->send($m[1], $args);
        elseif (preg_match('/^prepare([a-zA-Z0-9_]*)$/', $methodName, $m))
            return $mailer->prepare($m[1], $args);
        
        throw new Exception(get_class($this)."::$methodName() method does not exist");
    }
    ...
    public function prepare($methodName, $args)
    {
        ...
        if (!($mail instanceof Mail)) $mail = $this->create($methodName, $args);
        return $mail;
    }
    ...
    protected function render($templateName, $locals = array())
    {
        ...
        if (!file_exists($templatePath) || !is_readable($templatePath))
            throw new Exception("Missing template $templatePath");
    ...
    protected function getTemplatePath($templateName)
    {
        ...
        if (!isset(self::$templateRoot))
            throw new Exception('Template root not set');
    ...
    {
        return (isset(self::$transport)) ? self::$transport : new Transport\Sendmail();
    }
