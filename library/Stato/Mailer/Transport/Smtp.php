...
class Stato_Mailer_Transport_Smtp implements Stato_Mailer_Transport_ITransport
{
    const EOL = "\r\n";
    ...
    public function __construct($host, $config = array())
    {
        ...
        if (!in_array($config['auth'], array('plain', 'login', 'cram_md5', false)))
            throw new Stato_Mailer_Transport_Exception("Unknown authentication type: {$config['auth']}");
    ...
    public function connect()
    {
        ...
        if ($socket === false) {
            ...
            throw new Stato_Mailer_Transport_Exception($errorStr);
        }
        if (($result = stream_set_timeout($socket, $timeout)) === false)
            throw new Stato_Mailer_Transport_Exception('Could not set stream timeout');
    ...
    public function sendCommand($command, $expectedCode = null, $timeout = null)
    {
        if (!is_resource($this->socket))
            throw new Stato_Mailer_Transport_Exception('No connection has been established to '.$this->host);
        ...
        if ($result === false)
            throw new Stato_Mailer_Transport_Exception('Could not send command');
        ...
        if (!in_array($code, $expectedCode))
            throw new Stato_Mailer_Transport_Exception("Unexpected server response: $code $msg");
    ...
    public function getResponse()
    {
        ...
        if (!empty($info['timed_out']))
            throw new Stato_Mailer_Transport_Exception($this->host.' has timed out');

        if ($response === false)
            throw new Stato_Mailer_Transport_Exception('Could not read response');
    ...
    {
        throw new Stato_Mailer_Transport_Exception('Cram MD5 auth not implemented');
    }
}