...
class Smtp implements ITransport
{
    ...
    public function __construct($host, $config = array())
    {
        ...
        if (!in_array($config['auth'], array('plain', 'login', 'cram_md5', false)))
            throw new Exception("Unknown authentication type: {$config['auth']}");
    ...
    public function sendCommand($command, $expectedCode = null, $timeout = null)
    {
        if (!is_resource($this->socket))
            throw new Exception('No connection has been established to '.$this->host);
        ...
        if ($result === false)
            throw new Exception('Could not send command');
        ...
        if (!in_array($code, $expectedCode))
            throw new Exception("Unexpected server response: $code $msg");
    ...
    public function getResponse()
    {
        ...
        if (!empty($info['timed_out']))
            throw new Exception($this->host.' has timed out');
    ...
    {
        throw new Exception('Cram MD5 auth not implemented');
    }
}