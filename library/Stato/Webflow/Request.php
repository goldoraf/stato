...
class Request
{
    ...
    /**
     * Returns an instance or an array of UploadedFile instances
     * 
     * @return mixed
     */
 ...
 * @subpackage Webflow
 */
class RequestParams implements \ArrayAccess
{
    /**
     * Userland parameters
     * @var array
 ...
 * corresponds, you will get either an instance of UploadedFile or 
 * an array of instances.
 * 
 * @package Stato
 * @subpackage Webflow
 */
class RequestFiles implements \ArrayAccess
{
    protected $files;
    ...
    public function offsetSet($key, $file)
    {
        if (!$file instanceof UploadedFile)
            throw new \Exception('Only UploadedFile instances can be added to a RequestFiles instance');
    ...
    public function prepareFiles($files)
    {
        if (!is_array($files['name'])) {
            return new UploadedFile($files['tmp_name'], $files['name'], 
                                          $files['type'], $files['size'], $files['error']);
        } else {
            $uploads = array();
            foreach ($files['name'] as $k => $v) {
                $uploads[] = new UploadedFile($files['tmp_name'][$k], $v, 
