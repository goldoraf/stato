...
class Stato_Webflow_InvalidHttpMethod extends Exception {}
...
class Stato_Webflow_Request
{
    ...
    {
        $this->params = new Stato_Webflow_RequestParams();
        $this->files = new Stato_Webflow_RequestFiles();
    ...
    /**
     * Returns an instance or an array of Stato_Webflow_UploadedFile instances
     * 
     * @return mixed
     */
    ...
    public function getMethod()
    {
        ...
        if (!in_array($method, self::$allowedMethods))
            throw new Stato_Webflow_InvalidHttpMethod($method);
 ...
 * @subpackage Webflow
 */
class Stato_Webflow_RequestParams implements ArrayAccess
{
    /**
     * Userland parameters
     * @var array
 ...
 * corresponds, you will get either an instance of Stato_Webflow_UploadedFile or 
 * an array of instances.
 * 
 * @package Stato
 * @subpackage Webflow
 */
class Stato_Webflow_RequestFiles implements ArrayAccess
{
    protected $files;
    ...
    {
        if (!$file instanceof Stato_Webflow_UploadedFile)
            throw new Exception('Only Stato_Webflow_UploadedFile instances can be added to a Stato_Webflow_RequestFiles instance');
    ...
    public function prepareFiles($files)
    {
        if (!is_array($files['name'])) {
            return new Stato_Webflow_UploadedFile($files['tmp_name'], $files['name'], 
                                          $files['type'], $files['size'], $files['error']);
        } else {
            $uploads = array();
            foreach ($files['name'] as $k => $v) {
                $uploads[] = new Stato_Webflow_UploadedFile($files['tmp_name'][$k], $v, 
