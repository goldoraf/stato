...
class Stato_Webflow_ActionNotFound extends Exception {}
class Stato_Webflow_DoubleRespond extends Exception {}
class Stato_Webflow_MissingTemplate extends Exception {}

/**
 * Stato_Webflow_Controller class
 * 
...
class Stato_Webflow_Controller
{
     ...
     * Constructor
     * 
     * @param Stato_Webflow_Request $request
     * @param Stato_Webflow_Response $response
     * @return void
     */
    public function __construct(Stato_Webflow_Request $request, Stato_Webflow_Response $response)
    {
        $this->request = $request;
        $this->response = $response;
        $this->session = new Stato_Webflow_Session();
        
        $this->beforeFilters = new Stato_Webflow_FilterChain();
        $this->afterFilters  = new Stato_Webflow_FilterChain();
        $this->aroundFilters = new Stato_Webflow_FilterChain();
    }
    
    /**
     * Calls the action specified in the request object and returns a response
     * 
     * @return Stato_Webflow_Response
     */
    public function run()
    {
        $this->initialize();
        ...
        if (!$this->actionExists($action))
            throw new Stato_Webflow_ActionNotFound($action);
            
        $this->session->start();
        
        $this->flash = (isset($this->session['__FLASH__'])) ? $this->session['__FLASH__'] 
                                                            : new Stato_Webflow_Flash();
     ...
     * 
     * @param Stato_Webflow_ISessionHandler $handler
     * @return void
     */
    public function setSessionHandler(Stato_Webflow_ISessionHandler $handler)
    {
    ...
    protected function respond($data, $status = null)
    {
        if ($this->performedRespond)
            throw new Stato_Webflow_DoubleRespond();
    ...
    private function renderTemplate($template, $locals = array())
    {
        ...
        if (!is_readable($templatePath))
            throw new Stato_Webflow_MissingTemplate($templatePath);
    ...
    private function getTemplatePath($template)
    {
        ...
        throw new Stato_Webflow_MissingTemplate($template);
    }
