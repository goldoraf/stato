...
class Field
{
    ...
    public function getInput()
    {
        if (is_null($this->input)) {
            $inputClass = __NAMESPACE__.'\\'.$this->inputClass;
...
class CharField extends Field
{
    ...
    public function clean($value)
    {
        ...
        if (!is_null($this->length) && $length != $this->length)
            throw new ValidationError($this->errorMessages['length'], array($this->length, $length), $value);
            
        if (!is_null($this->minLength) && $length < $this->minLength)
            throw new ValidationError($this->errorMessages['min_length'], array($this->minLength, $length), $value);
            
        if (!is_null($this->maxLength) && $length > $this->maxLength)
            throw new ValidationError($this->errorMessages['max_length'], array($this->maxLength, $length), $value);
...
class IntegerField extends Field
{
    ...
    public function clean($value)
    {
        ...
        if (!is_null($this->minValue) && $value < $this->minValue)
            throw new ValidationError($this->errorMessages['min_value'], array($this->minValue), $value);
            
        if (!is_null($this->maxValue) && $value > $this->maxValue)
            throw new ValidationError($this->errorMessages['max_value'], array($this->maxValue), $value);
...
class BooleanField extends Field
{
    ...
    {
        if ($this->inputClass == 'CheckboxInput' || $this->input instanceof CheckboxInput) {
...
class ChoiceField extends Field
{
    ...
    public function clean($value)
    {
        ...
        if (!$this->isChoiceValid($value))
            throw new ValidationError($this->errorMessages['invalid_choice'], array($value));
    ...
    public function clean($value)
    {
        ...
        foreach ($value as $v) {
            if (!$this->isChoiceValid($v))
                throw new ValidationError($this->errorMessages['invalid_choice'], array($v));
        }
...
class FileField extends Field
{
    ...
    public function clean($value)
    {
        ...
        if ($this->isEmpty($value) || !$value instanceof \Stato\Webflow\UploadedFile) return null;
        ...
        switch ($value->error) {
            case \Stato\Webflow\UploadedFile::SIZE:
                $msg = $this->errorMessages['size'];
                break;
            case \Stato\Webflow\UploadedFile::NO_FILE:
