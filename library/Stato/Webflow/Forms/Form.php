...
class Form implements \Iterator
{
    ...
    {
        return new BoundField($this, current($this->fields), $this->key());
    }
    ...
    public function addField($name, $field, $options = array())
    {
        if (!$field instanceof Field) {
            $fieldClass = __NAMESPACE__.'\\'.$field.'Field';
            if (!class_exists($fieldClass, false))
                throw new Exception($fieldClass.' class not found');
                
            $field = new $fieldClass($options);
        }
        if ($field instanceof FileField) $this->multipart = true;
     ...
     * cleaned. Any ValidationError raised by this method will not be 
...
class BoundField
{
    ...
    public function __construct(Form $form, Field $field, $name)
    {
        $this->form = $form;
        $this->field = $field;
        $this->name = $name;
