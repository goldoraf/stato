...
class MigrationTest extends PHPUnit_Extensions_OutputTestCase
{
    ...
    public function teardown()
    {
        ...
        try { Person::connection()->remove_column('people', 'girlfriend'); }
        catch (Exception $e) { }
        try { Person::connection()->remove_column('people', 'exgirlfriend'); }
        catch (Exception $e) { }
    ...
    public function test_create_table_with_not_null_column()
    {
        ...
        $this->assertEquals('SInvalidStatementException', get_class($e));
    }
    ...
    public function test_create_table_with_defaults()
    {
        ...
        $this->assertEquals('hello', $columns['one']->default_value());
        $this->assertTrue($columns['two']->default_value());
        $this->assertFalse($columns['three']->default_value());
        $this->assertEquals(1, $columns['four']->default_value());
    }
    ...
    {
        Person::connection()->execute('TRUNCATE people');
        ...
        $this->assertEquals($neil->first_name, 'Neil');
        $this->assertEquals($neil->last_name, 'Armstrong');
        $this->assertEquals($neil->bio, 'First man on the moon...');
        $this->assertEquals($neil->age, 76);
        $this->assertEquals($neil->height, 1.72);
        $this->assertEquals($neil->birthday, new SDateTime(1930,8,5));
        $this->assertEquals($neil->favorite_day, new SDate(1969,07,21));
    ...
    {
        SActiveRecord::connection()->execute('TRUNCATE people');
        ...
        SMapper::reset_meta_information('Person');
        $p = Person::$objects->get($p->id);
        $this->assertEquals('bobette', $p->exgirlfriend);
    }
    ...
    public function test_rename_table()
    {
        ...
        $this->assertEquals('http://www.rubyonrails.org', $row['url']);
    ...
    public function test_change_column()
    {
        ...
        $this->assertEquals('integer', $old_columns['age']->type);
        ...
        $this->assertEquals('string', $new_columns['age']->type);
    }
    ...
    public function test_migrator()
    {
        ...
        $this->assertEquals(2, SMigrator::current_version());
        SMapper::reset_meta_information('Person');
        ...
        $r->save();
        $this->assertEquals('hello world', Reminder::$objects->get(1)->content);
        
        SMigrator::down(dirname(__FILE__).'/fixtures/migrate');
        
        $this->assertEquals(0, SMigrator::current_version());
        SMapper::reset_meta_information('Person');
        $this->assertFalse(in_array('last_name', array_keys(SActiveRecord::connection()->columns('people'))));
        $this->assertFalse(in_array('reminders', SActiveRecord::connection()->tables()));
    }
